...
// Función real para traer salas del backend (usa API_BASE, no localhost)
async function getRooms(token?: string): Promise<Room[]> {
  try {
    const accessToken = token ?? localStorage.getItem("accessToken");
    if (!accessToken) throw new Error("No hay token disponible");

    const url = new URL("/owners/me/studio/rooms", API_BASE).toString();

    const response = await fetch(url, {
      method: "GET",
      headers: { Authorization: `Bearer ${accessToken}` },
      // credentials: "include", // solo si usás cookies/sesión
      cache: "no-store",
    });

    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Rooms ${response.status}: ${errorText}`);
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
          ...
          <div className="bg-sky-800 text-white p-4 rounded-xl shadow-2xl">
            ...
            {loading ? (
              ...
              <p className="text-gray-300">No hay salas creadas.</p>
            ) : (
              ...
              >
                <div className="bg-gray-200 text-gray-500 text-center px-3 py-2 rounded-xl">
                  ...
                  <p className="text-xs uppercase">
                    {new Date(res.date).toLocaleString("es-AR", {
                      month: "short",
                    })}
                  </p>
                </div>
