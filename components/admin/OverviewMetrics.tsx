...
import { User, Building2, CalendarDays, Hourglass } from "lucide-react";

export default function OverviewMetrics() {
  const [loading, setLoading] = useState(true);
  const [totalUsers, setTotalUsers] = useState(0);
  const [activeStudios, setActiveStudios] = useState(0);
  const [pendingStudios, setPendingStudios] = useState(0);
    ...
    <section className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-2xl font-semibold text-sky-900">{loading ? "…" : fmt(totalUsers)}</p>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-2xl font-semibold text-sky-900">{loading ? "…" : fmt(activeStudios)}</p>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 flex items-center justify-between">
        <div>
          <p className="text-xs font-medium text-gray-500">Estudios pendientes</p>
          <p className="mt-2 text-2xl font-semibold text-sky-900">{loading ? "…" : fmt(pendingStudios)}</p>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-2xl font-semibold text-sky-900">{loading ? "…" : fmt(totalBookings)}</p>
        </div>
