    ...
    const filtered = useMemo(() => {
        ...
        if (firstTerm) {
            result = result.filter((studio) => {
                ...
                return (
                    c.includes(firstTerm) ||
                    p.includes(firstTerm) ||
                    co.includes(firstTerm)
                );
            });
        }
        if (selectedType && selectedType !== "Todos") {
            result = result.slice().sort((a, b) => {
                const matchA =
                    a.studioType?.toLowerCase() === selectedType.toLowerCase()
                        ? 1
                        : 0;
                const matchB =
                    b.studioType?.toLowerCase() === selectedType.toLowerCase()
                        ? 1
                        : 0;
        ...
        <section className="bg-white flex flex-col items-center w-full">
            ...
            <div className="w-full bg-sky-800 py-8 px-4 flex flex-col items-center">
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-white text-sm sm:text-base md:text-lg max-w-xl">
                        Busca y filtra entre miles de estudios de grabación
                        profesionales en la ubicación de tu preferencia
                    </p>
                </div>
            ...
            <div className="w-full max-w-[1200px] px-4 mt-8 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h3 className="text-lg md:text-xl font-semibold text-gray-800">
                    {filtered.length} estudios encontrados{" "}
                    {location && (
                        <span className="text-gray-600">en {location}</span>
                    )}
                </h3>
