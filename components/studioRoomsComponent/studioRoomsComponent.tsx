...
const AddInstrumentModal: FC<AddInstrumentModalProps> = ({ roomId, onClose, onAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const instrumentPayload = { ...formData, price: Number(formData.price) };
...
const RoomsGrid: FC = () => {
  ...
  const handleInstrumentAdded = (roomId: string, instrument: InstrumentType) => {
    setRooms((prev) =>
      prev.map((r) =>
        r.id === roomId
          ? { ...r, instruments: [...(r.instruments || []), instrument] }
          : r
      )
    );
  };
              ...
              <div key={room.id} className="rounded-xl border shadow-sm hover:shadow-xl transition flex flex-col">
                    ...
                    <div className="mt-2">
                      ...
                      <ul className="mt-1 space-y-1">
                        {room.instruments.map((inst) => (
                          <li key={inst.id} className="text-sm text-gray-600">- {inst.name} ({inst.categoryName})</li>
                        ))}
                      </ul>
                    </div>
                  )}
                ...
                <div className="p-4 border-t flex justify-between gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg border bg-sky-50 text-sky-700 hover:bg-sky-100"
                  >
                    <FaEdit /> Editar
                  </button>
                  <button
                    onClick={() => handleDelete(room.id)}
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg border bg-red-50 text-red-600 hover:bg-red-100"
                  >
                ...
                <div className="p-4 border-t">
                  <button
                    ...
                    className="w-full flex items-center justify-center px-3 py-2 rounded-lg bg-green-50 text-green-700 border hover:bg-green-100"
                  >
                    ＋ Agregar Instrumento
                  </button>
                </div>
