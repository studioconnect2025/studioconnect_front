// components/myStudio/MyStudioClient.tsx
"use client";
...
function collectImageUrlsDeep(input: any, acc: Set<string>) {
  ...
  if (typeof input === "object") {
    for (const [k, v] of Object.entries(input)) {
      if (
        ["url", "secure_url", "image", "imageUrl", "src", "path", "photo", "photoUrl", "cover", "coverPhoto", "banner", "thumbnail", "thumb"].includes(
          k
        ) &&
        typeof v === "string" &&
        isImageLikeUrl(v)
...
export default function MyStudioClient() {
  ...
  return (
    <div className="bg-white text-slate-900">
      <MyStudioHeader onEditStudio={() => setOpenEdit(true)} />
                ...
                {galleryPhotos.length
                  ? galleryPhotos.map((u, i) => (
                      <button
                        ...
                        className={`h-10 w-28 rounded-md border ${
                          i === photoIdx ? "border-sky-500 ring-1 ring-sky-300" : "border-slate-200"
                        } overflow-hidden`}
                        aria-label={`Foto ${i + 1}`}
                      >
                  ...
                  : Array.from({ length: 5 }).map((_, i) => (
                      <div
                        key={i}
                        className="h-10 w-28 rounded-md border border-slate-200 bg-slate-100 grid place-items-center text-[12px] text-slate-600"
                      >
                        {`Foto ${i + 1}`}
                      </div>
                    ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {(studio.services ?? []).length ? (
                    (studio.services as string[]).map((s, i) => (
                      <span
                        key={i}
                        className="rounded-md bg-slate-50 border border-slate-200 px-3 py-[6px] text-xs text-slate-700"
                      >
                        {s}
                      </span>
                    ))
