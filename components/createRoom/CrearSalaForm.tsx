...
const CrearSalaForm = () => {
  ...
  const [dias, setDias] = useState<{ dia: string; inicio: string; fin: string }[]>([]);
  ...
  const agregarDia = () => setDias([...dias, { dia: "Lunes", inicio: "09:00", fin: "22:00" }]);
  const eliminarDia = (index: number) => setDias(dias.filter((_, i) => i !== index));
  const handleDiasChange = (index: number, field: "dia" | "inicio" | "fin", value: string) => {
    setDias((prev) => {
      const updated = [...prev];
  ...
  return (
    <div className="w-screen bg-gray-50 p-8 overflow-auto mt-10">
      ...
      <div className="bg-white shadow-xl rounded-2xl p-6 w-full max-w-5xl mx-auto">
        <h2 className="text-3xl font-semibold text-sky-700 mb-4">Agregar Nueva Sala</h2>
        <p className="text-gray-600 mb-6">
          Crea una nueva sala de estudio con todos los detalles necesarios y especificaciones del equipo
        </p>
            ...
            try {
              ...
              const roomPayload = {
                ...
                availability: dias.reduce((acc, curr) => {
                  acc[curr.dia] = { start: curr.inicio, end: curr.fin };
                  return acc;
                }, {} as Record<string, { start: string; end: string }>),
              };

              const createdRoom = await roomsService.createRoom({ token, roomData: roomPayload });
        ...
        >
          {({ values, handleChange, setFieldValue, isSubmitting }) => (
            <Form className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Field
                    ...
                    className="w-full p-2 border rounded text-gray-700"
                  />
                  <ErrorMessage name="capacidad" component="div" className="text-red-500 text-sm" />
                </div>
                ...
                <div>
                  ...
                  <Field
                    ...
                    className="w-full p-2 border rounded text-gray-700"
                  />
                  <ErrorMessage name="tamano" component="div" className="text-red-500 text-sm" />
                </div>

                <div>
                  <label className="block text-gray-700">Tarifa por hora</label>
                  <Field name="tarifa">
                    {({ field }: any) => (
                      <input
                        {...field}
                        ...
                        onChange={(e) => {
                          ...
                          setFieldValue("tarifa", numericValue ? parseInt(numericValue) : 0);
                        }}
                        className="w-full p-2 border rounded text-gray-700"
                      />
                    )}
                  </Field>
                  <ErrorMessage name="tarifa" component="div" className="text-red-500 text-sm" />
                </div>

                <div>
                  <label className="block text-gray-700">Horas mínimas de reserva</label>
                  <Field
                    type="number"
                    name="reservaMinima"
              ...
              {/* Features */}
              <div>
                <label className="block text-gray-700 mb-2">Equipos</label>
                <div className="flex gap-2 flex-wrap">
                  {["Micrófono", "Amplificador", "Consola", "Acondicionamiento acústico", "Cables y conectores", "Interfaces de audio"].map((eq) => (
                    <button
                      type="button"
                      key={eq}
                      onClick={() => toggleEquipo(eq)}
                      className={`px-2 py-1 border rounded ${
                        equipos.includes(eq) ? "bg-sky-700 text-white" : "bg-white text-gray-700"
                      }`}
                    >
                      {eq}
                    </button>
              ...
              <div>
                <label className="block text-gray-700">Equipo personalizado</label>
                <Field
                  type="text"
              ...
              {/* Fotos */}
              <div>
                <label className="block text-gray-700">Fotos de la sala</label>
                <input
                  type="file"
                  multiple
                  accept="image/*"
              ...
              <div>
                <label className="block text-gray-700 mb-2">Disponibilidad de la sala</label>
                {dias.map((d, idx) => (
                  <div key={idx} className="flex items-center gap-2 mb-2">
                    <select
                      value={d.dia}
                      onChange={(e) => handleDiasChange(idx, "dia", e.target.value)}
                    ...
                    <input
                      type="time"
                      value={d.inicio}
                      onChange={(e) => handleDiasChange(idx, "inicio", e.target.value)}
                      className="p-1 border rounded text-gray-700"
                    />
                    <input
                      type="time"
                      value={d.fin}
                      onChange={(e) => handleDiasChange(idx, "fin", e.target.value)}
                      className="p-1 border rounded text-gray-700"
                    />
                    <button
                      type="button"
