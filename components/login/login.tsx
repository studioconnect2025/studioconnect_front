  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="rounded-t-xl bg-black text-white text-center py-3">
        <h1 className="text-base sm:text-lg font-bold">Inicio de sesión</h1>
      </div>

      {/* Form Container */}
      <div className="rounded-b-xl bg-white shadow-md p-4 sm:p-6">
        {/* Mensajes */}
        {error && (
          <p
            className="mb-2 text-center text-red-600 text-xs sm:text-sm"
            role="alert"
            aria-live="polite"
          >
            {error}
        ...
        {successMsg && (
          <p
            className="mb-2 text-center text-green-600 text-xs sm:text-sm"
            aria-live="polite"
          >
            {successMsg}
          </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1">
              <input
                ...
                required
                {...emailAria}
                className={`block w-full rounded-md border px-2 py-2 sm:px-3 sm:py-2 text-xs sm:text-sm text-black shadow-sm focus:border-sky-600 focus:ring-sky-600 ${
                  touchedEmail && emailErr ? "border-red-500" : "border-gray-300"
                }`}
              />
            </div>
            {touchedEmail && emailErr && (
              <p id="email-error" className="mt-1 text-[10px] sm:text-xs text-red-600">
                {emailErr}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            {touchedPassword && passErr && (
              <p id="password-error" className="mt-1 text-[10px] sm:text-xs text-red-600">
                {passErr}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
            <label className="text-gray-700 inline-flex items-center gap-2 text-xs sm:text-sm">
              <input
                type="checkbox"
                checked={remember}
            ...
            <Link
              ...
              onClick={() => onClose?.()}
              className="text-xs sm:text-sm text-sky-700 hover:underline"
            >
          ...
          <div className="relative my-2">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-2 text-[10px] sm:text-sm text-gray-500">
                O continuar con
              </span>
            </div>
          </div>
        ...
        {/* Registro */}
        <p className="mt-4 sm:mt-6 text-center text-[10px] sm:text-sm text-gray-600">
          ¿No tienes cuenta?{" "}
          <Link
