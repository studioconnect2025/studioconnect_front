...
export default function BookingPaymentClient({ bookingId }: { bookingId: string }) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const { clientSecret: cs } = await BookingService.payBooking(
          { bookingId },
          token
        );
        setClientSecret(cs);
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <p className="text-red-500">
          Falta <code>NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY</code> en el entorno.
        </p>
      </div>
    );
  }
  ...
  if (!clientSecret) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <p className="text-red-500">No se pudo obtener el client secret para esta reserva.</p>
      </div>
    );
  }
